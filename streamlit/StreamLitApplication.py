...
<p style="margin-bottom: 2.5em; margin-top:-0.5em; color:#000000;">
    ...
    st.runtime.legacy_caching.clear_cache()
    subtab_pre, subtab_bot = st.tabs(["**Présentation**","**ChatBot**"])
    ...
    def contient_chiffre(texte):
        ...
        Returns:
            Bool: True si l'entrée contient un digit False si non
        """
        for char in texte:
            if char.isdigit():
                return True
    ...
    def preparecomments(txt):
        data={"commentaire":[txt],"commentaireAmeliore":[' '.join(lemmatisation_etc(txt))]}
    ...
    with subtab_pre:
        st.write("Pour le Chat bot nous avons voulu comparer les résultats de nos modèles sur un jeu de données extèrieur au notre. Pour cela nous avons webscrappé les commentaire d'un dernier site de vente en ligne qui s'appelle Shein.")
        st.write("En appliquant nos modèles sur ce jeu de données nous avons obtenu les résultats suivant:")
        
        report_data = {'Precision': [0.44, 0.93], 'Recall': [0.63, 0.87], 'F1-score': [0.52, 0.90], 'Support': [198, 1205]}
        col1,col2=st.columns(2)
        with col1:
            classification_report = pd.DataFrame(report_data, index=[0, 1])
            st.write("Classification Report pour la Forêt Aléatoire:")
            st.table(classification_report)
            report_data = {'Precision': [0.77, 0.97], 'Recall': [0.86, 0.94], 'F1-score': [0.81, 0.96], 'Support': [254, 1149]}
            classification_report = pd.DataFrame(report_data, index=[0, 1])
            st.write("Classification Report pour la Regression Logistique:")
            st.table(classification_report)
        with col2:
            report_data = {'Precision': [0.59, 0.99], 'Recall': [0.93, 0.90], 'F1-score': [0.72, 0.94], 'Support': [180, 1223]}
            classification_report = pd.DataFrame(report_data, index=[0, 1])
            st.write("Classification Report pour le Réseau de neuronne:")
            st.table(classification_report)
            report_data = {'Precision': [0.60, 0.99], 'Recall': [0.92, 0.91], 'F1-score': [0.73, 0.95], 'Support': [185, 1218]}
            classification_report = pd.DataFrame(report_data, index=[0, 1])
            st.write("Classification Report pour le Bagging:")
            st.table(classification_report)
            
        st.write("Au vu de ces résultats, il semble logique que le bagging n'est pas si performant qu'on aurait pu l'èspéré du au fait que la forêt aléatoire se révèle bien trop mauvaise dans ce cas, de plus d'un point de vu pratique la regression logistique sera le modèle le plus rapide pour la prédiction. Pour une utilisation plus facile rapide et éfficace du Chat Bot, nous avons donc pour préféré mettre en place uniquement la regression.")
